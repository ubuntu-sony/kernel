...
static bool can_manipulate_uids(void)
{
	...
	return in_egroup_p(xt_qtaguid_ctrl_file->gid)
		|| unlikely(!from_kuid_munged(current_user_ns(), current_fsuid())) || unlikely(!proc_ctrl_write_limited)
		|| unlikely(uid_eq(current_fsuid(), xt_qtaguid_ctrl_file->uid));
}

static bool can_impersonate_uid(uid_t uid)
{
	return uid_eq(make_kuid(current_user_ns(), uid), current_fsuid()) || can_manipulate_uids();
}
...
static bool can_read_other_uid_stats(uid_t uid)
{
	...
	return in_egroup_p(xt_qtaguid_stats_file->gid)
		|| unlikely(!from_kuid_munged(current_user_ns(), current_fsuid())) || uid_eq(make_kuid(current_user_ns(), uid), current_fsuid())
...
static bool qtaguid_mt(const struct sk_buff *skb, struct xt_action_param *par)
{
	...
	if (sk != NULL) {
		...
		MT_DEBUG("qtaguid[%d]: filp...uid=%u\n",
			par->hooknum, filp ? from_kuid_munged(current_user_ns(), filp->f_cred->fsuid) : -1);
	}
	...
	sock_uid = from_kuid_munged(current_user_ns(), filp->f_cred->fsuid);
	if (do_tag_stat)
...
static int qtudev_open(struct inode *inode, struct file *file)
{
	...
	utd_entry = get_uid_data(from_kuid_munged(current_user_ns(), current_fsuid()), &utd_entry_found);
	if (IS_ERR_OR_NULL(utd_entry)) {
